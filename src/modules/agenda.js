...
export async function getAgendaConfig(empresaId) {
  ...
  return doc.data().agendaConfig || null;
}
...
export async function createBlock(empresaId, block) {
  ...
  const ref = await db.collection('empresas').doc(empresaId).collection('bloqueios').add(payload);
...
export async function checkConflict(empresaId, inicioISO, fimISO) {
  ...
  const agendamentosRef = db.collection('empresas').doc(empresaId).collection('agendamentos');
  const bloqueiosRef = db.collection('empresas').doc(empresaId).collection('bloqueios');

  // naive overlapping query: start < fim && end > inicio
  const agQuery = await agendamentosRef
    .where('inicio', '<', fimISO)
    .where('fim', '>', inicioISO)
    .get();

  if (!agQuery.empty) return true;

  const blQuery = await bloqueiosRef
    .where('inicio', '<', fimISO)
    .where('fim', '>', inicioISO)
    .get();

  if (!blQuery.empty) return true;
...
export async function generateSlotsForDate(empresaId, dateISO) {
  ...
  const dayName = new Date(dateISO).toLocaleDateString('pt-BR', { weekday: 'short' }).toLowerCase();
